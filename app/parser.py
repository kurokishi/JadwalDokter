...
class JadwalHafisParser:
    ...
    def _convert_to_standard_format(self, df):
        ...
        if records:
            ...
            # Calculate available flag
            def calc_available(row):
                working = str(row['working_hours']).strip() if pd.notna(row['working_hours']) else ''
                reguler = str(row['regular_schedule']).strip() if pd.notna(row['regular_schedule']) else ''
                eksekutif = str(row['executive_schedule']).strip() if pd.notna(row['executive_schedule']) else ''
                
                if (working not in ['', '[Reference]'] or 
                    reguler not in ['', '[Reference]'] or 
                    eksekutif not in ['', '[Reference]']):
                    return 1
                return 0
            
            result_df['available'] = result_df.apply(calc_available, axis=1)
            
            return result_df
        else:
            return pd.DataFrame()
    
    def _clean_time_value(self, value):
    ...
    def _create_doctor_records(self, ksm, doctor_name, jam_kerja, reguler, eksekutif):
